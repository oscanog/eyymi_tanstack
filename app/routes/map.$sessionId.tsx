...
function MapPage() {
  ...
  const meetupButtonLabel = isMeetingPlaceActive ? 'Change Meetup' : 'Set Meetup'
    ...
    <div className="relative h-screen w-full">
      ...
      {isMeetingPlaceFeatureEnabled && isPartnerConnected ? (
        <button
          ...
          className={`absolute right-4 bottom-[max(108px,var(--safe-area-bottom))] z-[420] rounded-full px-4 py-3 flex items-center gap-2 meeting-fab ${isMeetingPlaceActive ? 'meeting-fab--active' : ''} disabled:opacity-55 disabled:cursor-not-allowed`}
          aria-label={meetupButtonLabel}
        >
          {isMeetingPlaceActive ? (
            <RefreshCw className="w-4 h-4" />
          ) : (
            <MapPin className="w-4 h-4" />
          )}
          <span className="text-sm font-semibold">{meetupButtonLabel}</span>
        </button>
      ) : null}
